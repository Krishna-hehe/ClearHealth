...
class MultiTrendChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isNormalized) {
      ...
      chartData = _buildDualAxisChart(keys, colors); // Not fully supported by fl_chart out of box easily without side titles trickery
        ...
        touchTooltipData: LineTouchTooltipData(
          ...
          getTooltipItems: (touchedSpots) {
            return touchedSpots.map((LineBarSpot touchedSpot) {
              ...
              final test = report.testResults?.firstWhere((t) => t.name == testName, orElse: () => TestResult(name: '', loinc: '', result: '', unit: '', reference: '', status: ''));
  ...
  LineChartData _buildDualAxisChart(List<String> keys, List<Color> colors) {
    ...
    for (var p in s1) {
      final test = p.testResults?.firstWhere((t) => t.name == keys[0], orElse: () => TestResult(name: '', loinc: '', result: '', unit: '', reference: '', status: ''));
    ...
    for (var p in s2) {
      final test = p.testResults?.firstWhere((t) => t.name == keys[1], orElse: () => TestResult(name: '', loinc: '', result: '', unit: '', reference: '', status: ''));
      final v = double.tryParse(test?.result ?? '') ?? 0;
      if (v < min2) min2 = v;
      if (v > max2) max2 = v;
    ...
    return LineChartData(
      lineBarsData: [
        LineChartBarData(spots: spots1, color: colors[0], isCurved: true, barWidth: 3),
        LineChartBarData(spots: spots2, color: colors[1], isCurved: true, barWidth: 3),
      ],
      titlesData: FlTitlesData(
        leftTitles: AxisTitles(
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (val, meta) => Text(val.toStringAsFixed(1), style: TextStyle(color: colors[0], fontSize: 10)),
          )
        ),
        rightTitles: AxisTitles(
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (val, meta) {
              ...
              final original = (val - min1) / (max1 - min1) * (max2 - min2) + min2;
              return Text(original.toStringAsFixed(1), style: TextStyle(color: colors[1], fontSize: 10));
            }
          )
        ),
         bottomTitles: AxisTitles(
        ...
        touchTooltipData: LineTouchTooltipData(
          getTooltipItems: (touchedSpots) {
             return touchedSpots.map((spot) {
               ...
               final point = normalizedData![key]!.firstWhere((p) => p.date.millisecondsSinceEpoch == spot.x.toInt(), orElse: () => normalizedData![key]!.first);
