...
class SupabaseService {
  ...
  Future<void> createProfile(Map<String, dynamic> data) async {
    ...
    final profileData = {..._sanitizeMap(data), 'user_id': client.auth.currentUser!.id};
  ...
  Future<void> createLabResult(Map<String, dynamic> data) async {
    ...
    try {
      ...
      // Recursively sanitize the test results
      final sanitizedTestResults = testResults.map((test) {
        if (test is Map<String, dynamic>) {
          return _sanitizeMap(test);
        }
        return test;
      }).toList();

      final abnormalCount = sanitizedTestResults.where((t) {
      ...
      final sanitizedLabName = validator.sanitizeForDb(
        data['lab_name']?.toString() ?? 'Manual Upload',
      );
      final sanitizedStoragePath = data['storage_path'] != null
          ? validator.sanitizeForDb(data['storage_path'])
          : null;
      ...
      await client.from('lab_results').insert({
        ...
        'status': status,
        'test_count': sanitizedTestResults.length,
        'abnormal_count': abnormalCount,
        'test_results': sanitizedTestResults,
